...
public class Scheduler {
    ...
    private static long fiveMinForward(long time) {
        ...
        return (time / 300000 + 1) * 300000;
>>>>>>> c1b2a371aa603a15a73ff727b317570a2f063bae
    }
...
<<<<<<< HEAD
        //takes records the start and end times of each todo or event in separate ArrayLists. Times are in chronological ascending order. 
        for(int i = 0; i < eventStart.length + setTodos.size(); i++)
        {


            //makes sure that if one list is exhausted, the loop will only take the start/end times from the other list
            ...
            //if the next todo comes first, then add the start/end times of the todo to the Arraylists. Otherwise, add the start/end times of the next event.
